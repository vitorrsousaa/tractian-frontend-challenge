...
export function Home() {
	...
	const buildTree = useCallback((locations: Location[], assets: Asset[]) => {
	...
	const filterTree = useCallback(
		(tree: ItemTree[], filter: { sensor: boolean; status: boolean }) => {
			return tree.map((item) => {
				const sensorFilterIsActivated =
					filter.sensor && item.sensorType === "energy";
				const statusFilterIsActivated =
					filter.status && item.status === "alert";

				const isBeingFiltered =
					sensorFilterIsActivated || statusFilterIsActivated;
